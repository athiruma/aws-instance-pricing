...
class InstanceTypes:
    ...
    def instance_prices(self):
        ...
        for region in regions:
            region_pricing = {}
            instance_types = self.get_instance_types(region_name=region['RegionName'])
            for instance_type in instance_types:
                price = self.instance_price(region_name=region['RegionName'], instance_type=instance_type)
                if float(price) > 0:
                    region_pricing[instance_type] = round(float(price), 4)
            aws_pricing[region['RegionName']] = region_pricing
        with open('instance_pricing.json', 'w') as file:
            json.dump(aws_pricing, file, indent=4)
